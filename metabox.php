...
function liblynx_meta_box_callback($post)
{
    ...
    switch ($liblynx_setting) {
        case 'nothing':
            echo '<p>Liblynx not activated - no additional authentication required to view this content</p>';
            break;
        case 'all':
            echo '<p>All pages and posts require authentication via LibLynx</p>';
            break;
        case 'tagged':
            ...
            echo '<label for="liblynx_protect">Check to ensure visitors must be authenticated via LibLynx to see this content</label>';
        ...
        case 'untagged':
            echo '<input type="hidden" name="liblynx_metabox" value="1">';
            echo '<input type="checkbox" id="liblynx_protect" name="liblynx_protect" value="1" '.$checked.'/>';
            echo '<label for="liblynx_protect">Check to ensure visitors can view this content <u>without</u> authenticating via LibLynx.</label>';
            echo '<p><i>The current LibLynx plugin settings mean that all pages and posts must be authenticated - check the box above only for freely available pages and posts.</i></p>';
            echo '<div id="liblynx_protect_invert">';
            echo '<label for="liblynx_custom_unit">Visitor must have subscription for </label>';
            echo '<input type="text" size="8" id="liblynx_custom_unit" name="liblynx_custom_unit" placeholder="'.htmlentities($default_code).'" value="'.htmlentities($unit).'">';
            echo '</div>';
            break;
    ...
    echo '<p>See <a href="options-general.php?page=liblynx-connect-admin">settings</a> to change LibLynx configuration settings</p>';
    ...
    function updateLibLynxMetabox()
    {
        ...
        if (enabled) {
            ...
            jQuery('#liblynx_protect_invert').hide();
        } else {
            jQuery('#liblynx_protect_options').hide();
            jQuery('#liblynx_protect_invert').show();
        }
    }
